...
import { foodsApi, cuisinesApi } from '@/api'
...
const filtered = computed(() => {
  ...
  if (filterCuisineId.value) list = list.filter(i => i.cuisineId === filterCuisineId.value)
  if (search.value) {
...
async function load() {
  loading.value = true
  try {
    const [itemsRes, cuisinesRes] = await Promise.all([foodsApi.getAll(), cuisinesApi.getAll()])
...
async function saveItem(payload) {
  if (editTarget.value) await foodsApi.update(editTarget.value.id, payload)
  else                  await foodsApi.create(payload)
...
async function confirmDelete() {
  await foodsApi.delete(deleteTarget.value.id)
...
<template>
  <div class="max-w-7xl mx-auto px-4 sm:px-6 py-8">
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
      <div>
        <h1 class="text-2xl font-bold text-white flex items-center gap-2">🍜 Foods</h1>
        <p class="text-white/40 text-sm mt-1">{{ items.length }} dishes available</p>
      </div>
      <button v-if="auth.isAdmin" @click="editTarget = null; showForm = true" class="btn-primary flex items-center gap-2">
        <span class="text-lg">+</span> Add food
      </button>
    </div>
    ...
    <div class="flex flex-col sm:flex-row gap-3 mb-6">
      <div class="relative flex-1">
        ...
        <input v-model="search" class="input pl-9" placeholder="Search foods..." />
      </div>
      <select v-model="filterCuisineId" class="input sm:w-48">
    ...
    <div v-else class="text-center py-20">
      ...
      <p class="text-white/40 text-lg">No foods found</p>
    </div>

    <ItemFormModal :show="showForm" type="foods" :item="editTarget" :cuisines="cuisines" @close="showForm = false" @save="saveItem" />
    <ConfirmModal  :show="showConfirm" :message="`Delete '${deleteTarget?.title}'?`" @confirm="confirmDelete" @cancel="showConfirm = false" />
  </div>
</template>
