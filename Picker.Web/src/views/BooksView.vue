...
import { booksApi, genresApi } from '@/api'
...
async function load() {
  loading.value = true
  try {
    const [itemsRes, genresRes] = await Promise.all([booksApi.getAll(), genresApi.getAll()])
...
async function saveItem(payload) {
  if (editTarget.value) await booksApi.update(editTarget.value.id, payload)
...
async function confirmDelete() {
  await booksApi.delete(deleteTarget.value.id)
...
<template>
  <div class="max-w-7xl mx-auto px-4 sm:px-6 py-8">
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
      <div>
        <h1 class="text-2xl font-bold text-white flex items-center gap-2">📚 Books</h1>
        <p class="text-white/40 text-sm mt-1">{{ items.length }} books available</p>
      </div>
    ...
    <div class="flex flex-col sm:flex-row gap-3 mb-6">
      <div class="relative flex-1">
        ...
        <input v-model="search" class="input pl-9" placeholder="Search books..." />
      </div>
    ...
    <ItemFormModal :show="showForm" type="books" :item="editTarget" :genres="genres" @close="showForm = false" @save="saveItem" />
